...
class MulticompanyConfig(models.AbstractModel):
    ...
    def _special_default_user(self):
        default_users = self.env['res.users'].sudo().with_context(active_test=False).search(['|', ('login', '=', 'default'), ('login', '=like', 'default_user_for_company_%')])
    ...
    def _special_account_chart_template(self):
        ...
        for tup in template_list:
            ...
            if not template:
                template = generic_template.copy({'name': name, 'code_digits': code_digits})
            if country_code:
                companies = self.env['res.company'].sudo().search([('chart_template_id', '=', False), ('partner_id.country_id.code', '=', country_code)])

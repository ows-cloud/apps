...
class MulticompanyConfig(models.AbstractModel):
    ...
    def _special_uom(self):
        """
        File "/o14/o14-server/addons/uom/models/uom_uom.py",
            line 85, in _check_category_reference_uniqueness
        ...
        for category in categories:
            ...
            for cat_ref_uom in cat_ref_uoms.filtered(lambda u: u.company_id.id != 1):
                ...
                # and reference all the related company uoms to this category.
    ...
    def _special_account_chart_template(self):
        """
        File "/o14/custom/openupgrade/openupgrade_scripts/scripts/account/14.0.1.1/
            post-migration.py", line 481, in fill_statement_lines_with_no_move
            "partner_id",
        File "/o14/o14-server/addons/account/models/account_bank_statement.py",
            line 994, in _synchronize_to_moves
            line_vals_list = self._prepare_move_line_default_vals()
        File "/o14/o14-server/addons/account/models/account_bank_statement.py",
            line 740, in _prepare_move_line_default_vals
            ) % self.journal_id.display_name)
        odoo.exceptions.UserError: You can't create a new statement line
            without a suspense account set on the Bank journal.
        """

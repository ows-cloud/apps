...
class IrModule(models.Model):
    ...
    mig_info_ids = fields.One2many(
        "ir.module.migration.info", "module_id", string="Migration Info"
    )
        ...
        for repo in github_repo:
            for module_name in repo:
                if module_name in odoo_modules:
                    ir_module_module = self.env["ir.module.module"].search(
                        [("name", "=", module_name)], limit=1
                    )
                    if ir_module_module:
                        for module_version in repo[module_name]:
                            ir_module_migration_info = self.env[
                                "ir.module.migration.info"
                            ].search(
                                [
                                    "&",
                                ...
                                ir_module_migration_info.update(
                                    {
                                        ...
                                        "mig_version": module_version,
                                        "mig_title": repo[module_name][
                                            module_version
                                        ].get("mig_title", ""),
                                        "mig_url": repo[module_name][
                                            module_version
                                        ].get("mig_url", ""),
                                        "mig_status": repo[module_name][
                                            module_version
                                        ].get("mig_state", ""),
                                        "mig_opened": repo[module_name][
                                            module_version
                                        ].get("mig_opened", ""),
                                        "mig_merged": repo[module_name][
                                            module_version
                                        ].get("mig_merged", ""),
                                        "mig_no_of_reviewers": repo[module_name][
                                            module_version
                                        ].get("mig_no_of_reviewers", ""),
                                        "mig_no_of_comments": repo[module_name][
                                            module_version
                                        ].get("mig_no_of_comments", ""),
                                    }
                                )
                            else:
    ...
    def get_github_repo(self, reponame):
        ...
        # since graphql returns only 100 result at a time,
        # then get the next query based on its cursor
        ...
        while getPage:
            ...
            getPage = result["data"]["repository"]["pullRequests"]["pageInfo"][
                "hasNextPage"
            ]
            cursor = (
                'after:"'
    ...
    def run_query(self, query):
        ...
        self.env["ir.config_parameter"].sudo().get_param("oca_username", False)
        if not oca_token:
        ...
        for pr in nodes:
            ...
            if "MIG" in title:
                ...
                try:
                    ...
                    if module != "":
                        if re.search(
                            r"([^\[MIG\]\[15\.0\]]|[^\[MIG\]\[15\.0\]])\s*{}\s*:.*$".format(
                                module
                            ),
                            title,
                        ):
                            if pr["state"] in ("OPEN", "MERGED"):
                                # If multiple PRs are found,
                                # select the PR with the lowest ID number.
                                if not module_pr_id or module_pr_id > pr["number"]:
                                    ...
                                    res[version] = {
                                        ...
                                        "mig_no_of_reviewers": pr["reviews"][
                                            "totalCount"
                                        ],
                                        "mig_no_of_comments": pr["comments"][
                                            "totalCount"
                                        ]

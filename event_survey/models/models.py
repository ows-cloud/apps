...
class EventEvent(models.Model):
    ...
    registration_partner_id = fields.Many2one('res.partner', "Customer",
        help="Public users (no login) will skip address to simplify the payment. This partner will be used for the sales order.")
    registration_name_question_id = fields.Many2one('survey.question', "Question: Name", help="Single line text box. Mandatory answer.")
    registration_email_question_id = fields.Many2one('survey.question', "Question: Email", help="Single line text box. Mandatory answer. Input must be an email.")
    ...
    registration_products_label_ids = fields.One2many('survey.label', 'question_id', "Question: Product Labels", related='registration_products_question_id.labels_ids')
...
class EventRegistration(models.Model):
    ...
    survey_user_input_print_url = fields.Char(string="Survey Answers URL", related='survey_user_input_id.answers_url')
...
class SurveyUserInput(models.Model):
    ...
    order_id = fields.Many2one('sale.order', "Sales Order", help="Do multiple surveys before ")
...
class SurveySurvey(models.Model):
    ...
    def _get_event(self):
        for record in self:
            record.event_id = self.env['event.event'].search([('registration_survey_id','=',record.id)])

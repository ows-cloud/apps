...
class HrPayslipLine(models.Model):
    ...
    def l10n_no_feriepenger(self):
        ...
        fp_prosent = float(self.env['res.field.value'].search(
            [('model','=','res.company'),('field_id','=',self.env.ref('l10n_no_payroll.res_field_l10n_no_fp_prosent').id)]
        ).ensure_one().value) / 100
        fp_prosent_senior = float(self.env['res.field.value'].search(
            [('model','=','res.company'),('field_id','=',self.env.ref('l10n_no_payroll.res_field_l10n_no_fp_prosent_senior').id)]
        ).ensure_one().value) / 100
        ...
        loennsarter_beregn_fp_ids = [r.res_id for r in self.env['res.field.value'].search(
            [('model','=','hr.salary.rule'),('field_id','=',self.env.ref('l10n_no_payroll.res_field_l10n_no_BeregnFP').id)]
        )]

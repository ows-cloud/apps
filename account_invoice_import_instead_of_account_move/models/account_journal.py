    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('type') == 'purchase' and vals.get('alias_name'):
                ...
                company_purchase_journal_with_mail_alias[company_id] += existing[0].name if existing else ''
                # create or write
                if company_purchase_journal_with_mail_alias[company_id]:
                    raise UserError("You can only have one purchase journal with alias, and journal {} already has an alias.".format(
                        company_purchase_journal_with_mail_alias[company_id]))
                self.env['res.company'].browse(company_id).invoice_import_email = vals.get('alias_name')
                company_purchase_journal_with_mail_alias['company_id'] += vals.get('name', 'new')
        ...
        if vals.get('alias_name'):
            ...
            if journal_type == 'purchase':
                ...
                if company_purchase_journal_with_mail_alias:
                    raise UserError("Journal {} already has an alias, and you can only have one purchase journal with alias.".format(

        ...
        for vals in vals_list:
            if vals.get("type") == "purchase" and vals.get("alias_name"):
                ...
                if company_purchase_journal_with_mail_alias[company_id]:
                    raise UserError(
                        _(
                            "You can only have one purchase journal with alias, " + \
                                "and journal {} already has an alias."
                        ).format(company_purchase_journal_with_mail_alias[company_id])
                    )
        ...
        if vals.get("alias_name"):
            ...
            if journal_type == "purchase":
                ...
                if company_purchase_journal_with_mail_alias:
                    raise UserError(
                        _(
                            "Journal {} already has an alias, and you can only " + \
                                "have one purchase journal with alias."
                        ).format(company_purchase_journal_with_mail_alias)
                    )
                self.env["res.company"].browse(
                    company_id
                ).invoice_import_email = vals.get("alias_name")

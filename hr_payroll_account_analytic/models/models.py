        ...
        for group in grouped_payslips:
            ...
            for slip in self.browse(group['payslips']):
                ...
                for line in line_ids:
                    ...
                    if self.env.user.company_id.country_id.code == 'NO':
                        # TODO: move to a method in l10n_no_hr_payroll; call the method from here (depends on def _add)
                        c_field = {}
                        c_records = self.env.user.company_id.field_value_hr_ids

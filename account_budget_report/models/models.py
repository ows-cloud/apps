...
# copied from
# https://github.com/odoo/odoo/blob/11.0/addons/account_budget/models/account_budget.py
class CrossoveredBudget(models.Model):
    ...
    #    'crossovered_budget_id', 'Budget Lines',
...
class AccountBudgetLine(models.Model):
    ...
    def create(self, values):
        ...
        if values.get("crossovered_budget_id"):
            ...
            budget = self.env["crossovered.budget"].browse(
                values["crossovered_budget_id"]
            )
    ...
    def write(self, values):
        ...
        for record in self:
            ...
            if values and "crossovered_budget_id" in values:
                budget = (
                    self.env["crossovered.budget"].browse(
                        values["crossovered_budget_id"]
                    )
                    if values.get("crossovered_budget_id")
                    else None
            ...
            if "balance" in values:
                if not record.balance_history:
                    ...
                    values["balance_history"] = (
                        record.balance_history + ", " + "%.2f" % record.balance
                    )
    ...
    def _check_date(self, date, crossovered_budget_id):
        ...
        if date <= (lock_date):
            message = _(
                "You cannot add/modify budget lines prior to and inclusive of " + \
                "the end date %s of the budget."
            ) % (lock_date)
    ...
    def _replace_budget_lines(self, records, change_date):
        ...
        for record in records:
            budget_date = (
                datetime.strptime(record.date, "%Y-%m-%d") + change_date
            ).strftime("%Y-%m-%d")
            csv += (
                ",".join(
                    [
                        ...
                        str(record.analytic_account_id.id),
                        str(record.crossovered_budget_id.id),
                        str(record.balance),
                    ]
                )
                + "\n"
        ...
        df = (
            ...
            .groupby(
                ["date", "account_id", "analytic_account_id", "crossovered_budget_id"]
            )["balance"]
            .sum()
        )
...
class AccountBudgetReport(models.Model):
    ...
    analytic_user_id = fields.Many2one(
        "res.users", string="Analytic Manager", store=True
    )
    include_initial_balance = fields.Boolean(
    ...
    accounting_or_budget = fields.Selection(
        selection=[("accounting", "Accounting"), ("budget", "Budget")],
        string="Source",
        store=True,
    )

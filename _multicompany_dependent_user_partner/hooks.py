...
def pre_init_hook(cr):
    ...
    partner_field = (
        env["ir.model.fields"]
        .search([("model", "=", "res.users"), ("name", "=", "partner_id")])
        .ensure_one()
    )
    ip_partners = env["ir.property"].search(
        [("fields_id", "=", partner_field.id), ("company_id", "=", False)]
    )
...
def uninstall_hook(cr, registry):
    ...
    ip_partners = env["ir.property"].search(
        [("fields_id", "=", partner_field.id), ("company_id", ">", 0)]
    )
    for ip_partner in ip_partners:
        user_id = ip_partner.res_id.split(",")[1]

...
class HrContract(models.Model):
    ...
    def _compute_default_field_value_ids(self):
        result = []
        records = self.env["res.field"].search(
            [
                ("model", "=", "hr.contract"),
                ("auto_create", "=", True),
                "|",
...
class ResCompany(models.Model):
    ...
    field_value_hr_ids = fields.One2many(
        "res.field.value",
        "res_id",
        string="Fields",
        default=_compute_default_field_value_ids,
        domain=[("model", "=", "res.company"), ("field_app", "=like", "hr%")],
        copy=True,
    )

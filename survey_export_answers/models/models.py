...
class SurveySurvey(models.Model):
    ...
    def export_answers(self):
        ...
        all_answers = [
            [input.id, json.loads(self.prefill(survey, input.access_token))]
            for input in survey.user_input_ids
        ]
        ...
        for q in questions:
            question_list.append(
                (
                    "%s_%s_%s" % (q.survey_id.id, q.page_id.id, q.id),
                    q.title,
                    q.question_type,
                    q.page_id.sequence,
                    q.page_id.id,
            ...
            for key, question, my_type, t3, t4, t5, t6 in question_list:
                ...
                if question_answer:
                    if my_type in ("simple_choice", "multiple_choice", "matrix"):
                        ...
                        csv_answer = '"%s"' % ", ".join(
                            [label.value for label in answer_labels]
                        )
                    else:
                        assert (
                            len(question_answer) == 1
                        ), "%s answers to the question %s" % (
                            len(question_answer),
                            question,
                        )
    ...
    # odoo.addons.survey.controllers.main.WebsiteSurvey().prefill(survey, input.token)
    # does not return JSON but <Response 61 bytes [200 OK]> because of @http.route()
    def prefill(self, survey, token, page=None, **post):
        ...
        if page:
            previous_answers = UserInputLine.sudo().search(
                [("user_input_id.access_token", "=", token), ("page_id", "=", page.id)]
            )
        else:
            previous_answers = UserInputLine.sudo().search(
                [("user_input_id.access_token", "=", token)]
            )
        ...
        for answer in previous_answers:
            if not answer.skipped:
                ...
                if answer.answer_type == "text_box":
                    ...
                    answer.answer_type == "char_box"
                    and answer.question_id.question_type == "char_box"
                ):
                ...
                if answer_value:
                    ...
                    _logger.warning(
                        "[survey] No answer found for question %s marked as non skipped"
                        % answer_tag
                    )
        _logger.warning(
            "prefill: ret = %s, json.dumps(ret) = %s" % (ret, json.dumps(ret))
        )

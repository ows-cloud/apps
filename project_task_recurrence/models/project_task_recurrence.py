...
class Recurrences(models.Model):
    ...
    recurring_fields = fields.Char(
        "Recurring fields",
        help="With ['user_id', 'priority', 'date_deadline'] the next task will be " + \
            "assigned to the same user, with the same priority, with the next deadline.",
    )

    # TODO: Fix: "Error, a partner cannot follow twice the same object."
    # TODO: Archive recurring tasks. "You cannot archive recurring tasks.
    #   Please, disable the recurrence first."
    ...
    #         create_values['date_deadline'] = self._get_next_recurring_dates(
    #           tomorrow, self.repeat_interval, self.repeat_unit, self.repeat_type,
    #           self.repeat_until, self.repeat_on_month, self.repeat_on_year,
    #           self._get_weekdays(), self.repeat_day, self.repeat_week,
    #           self.repeat_month, count=1)
    #     return create_values

    # "'project.task.recurrence' object has no attribute 'user_id'"
    # psycopg2: "invalid input syntax for type date:
